...
const EventManagement = () => {
  ...
  const handleUpdateEvent = async (e) => {
    ...
    try {
      ...
      // If we have a new banner, upload it now
      if (croppedBannerBlob || (bannerPreview && !croppedBannerUrl.includes(API.replace('/api', '')))) {
        ...
        // If no new banner, just refresh events
        await fetchEvents();
      }
  ...
  const handleBannerUpload = (e) => {
    ...
    reader.onload = (e) => {
      ...
      setCroppedBannerUrl(null);
      // Initialize with a default crop that covers the entire image
      setCrop({
        unit: '%',
        x: 0,
        y: 0,
      ...
      // Set completed crop to the initial crop so cropping works even without user interaction
      setCompletedCrop({
        unit: '%',
        x: 0,
        y: 0,
  ...
  const handleCropComplete = async () => {
    ...
    console.log('Crop to use:', cropToUse);
  ...
  const saveBanner = async () => {
    if (!croppedBannerUrl && !bannerPreview) {
      toast.error("No banner image selected");
      return;
    }

    // In a real implementation, you would upload the banner to the server here
    toast.success("Banner ready to be saved with event!");
  };
