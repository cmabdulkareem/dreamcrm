...
const AppHeader = () => {
  ...
  const navigationItems = [
    { title: "Dashboard - Leads Overview", path: "/", category: "Dashboard", icon: "chart" },
    { title: "Dashboard - Students Overview", path: "/students-overview", category: "Dashboard", icon: "users" },
    { title: "Dashboard - Revenue Overview", path: "/revenue-overview", category: "Dashboard", icon: "dollar" },
    { title: "New Lead", path: "/new-lead", category: "Lead Management", icon: "plus" },
    { title: "Manage Leads", path: "/lead-management", category: "Lead Management", icon: "list" },
  ...
  const getIcon = (iconName) => {
    const icons = {
      ...
      target: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" /></svg>,
      ...
      button: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122" /></svg>,
      image: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>,
      video: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      results.push(...navResults.slice(0, 5));
      ...
      try {
        ...
        const customerResults = customers
          ...
          .map(customer => ({
            ...
            subtitle: `${customer.phone1}${customer.email ? ' • ' + customer.email : ''}`,
  ...
  const handleResultClick = (result) => {
    if (result.type === "customer") {
      // Store customerId in sessionStorage to open the edit modal
      sessionStorage.setItem("openLeadId", result.customerId);
    }
  ...
  return (
    <header className="sticky top-0 flex w-full bg-white border-gray-200 z-99999 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
      <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full lg:w-auto gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          <button
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search modules, leads, or data..."
                ...
                {showResults && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg shadow-lg max-h-[400px] overflow-y-auto z-50">
                    {isSearching ? (
                      ...
                      <div className="py-2">
                        {searchResults.map((result, index) => (
                          <button
                            ...
                            className="w-full px-4 py-2.5 text-left hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors flex items-center gap-3 border-b border-gray-100 dark:border-gray-800 last:border-b-0"
                          >
                            <span className="text-gray-500 dark:text-gray-400">{getIcon(result.icon)}</span>
